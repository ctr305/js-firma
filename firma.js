...
botonContrato.addEventListener('click', (e) => {
    ...
    const ventana = window.open('contrato.html', 'Contrato');
});
...
function OnClicODedoLevantado(e) {
    modoEscritura = false;
}

['mousedown', 'touchstart'].forEach((evento) => {
    canvas.addEventListener(evento, OnClicOToqueIniciado, { passive: true });
});

['mousemove', 'touchmove'].forEach((evento) => {
    canvas.addEventListener(evento, OnMouseODedoMovido, { passive: true });
});

['mouseup', 'touchend'].forEach((evento) => {
    canvas.addEventListener(evento, OnClicODedoLevantado, { passive: true });
});
